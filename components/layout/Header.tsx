  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[1000] flex lg:hidden w-full items-center justify-between px-6 py-4 font-sans text-sm text-neutral-400">
      <nav className="flex items-center">
        <Link href="/" className={linkClass} onClick={e => { if (pathname === '/') e.preventDefault(); closeAbout() }}>projects</Link>
        <span>,&nbsp;</span>
        <Link href="/lab" className={linkClass} onClick={e => { if (pathname === '/lab') e.preventDefault(); closeAbout() }}>lab</Link>
        <span>,&nbsp;</span>
